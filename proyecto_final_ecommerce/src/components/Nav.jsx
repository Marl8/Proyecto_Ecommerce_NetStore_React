  ...
  return (
    <nav style={{backgroundColor: 'black', color: 'white'}}>
      <ul>
        ...
        <li><Link className='link' to={'/nosotros'}>Acerca de</Link></li>
        <li><Link className='link' to={'/contacto'}>Contacto</Link></li>
        <li className='nav-count-items'>Items: {cartCount}</li>
        <li>
          <button className='btnCart-nav' onClick={()=> {setCartOpen(true)}}>
            <i className="fa-solid fa-cart-shopping"></i>
          </button>  
            <Cart cart={cart} vaciarCart= {vaciarCarrito} borrarProducto={borrarProducto} isOpen={isCartOpen} onClose={()=>{setCartOpen(false)}}/>          
        </li>
      </ul>
    </nav>
  )
